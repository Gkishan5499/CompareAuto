...
export default function BrandCSVImport() {
  ...
  const onUpload = async () => {
    ...
    Papa.parse(selectedFile, {
      ...
      complete: async (results) => {
        try {
          await client.post('/api/brands/bulk', { data: results.data }, {
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-2xl font-semibold">Import Brands (CSV)</h2>
      <Card className="p-4">
        <div className="border-dashed border-2 rounded p-6 text-center" onDrop={onDrop} onDragOver={(e)=>e.preventDefault()}>
          ...
          <div className="text-sm text-muted-foreground mt-1">columns: id,name,logo,country,slug</div>
        </div>
        {preview?.length > 0 && (
