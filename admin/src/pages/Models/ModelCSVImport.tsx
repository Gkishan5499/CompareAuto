...
export default function ModelCSVImport() {
  ...
  const onUpload = async () => {
    ...
    Papa.parse(selectedFile, {
      ...
      complete: async (results) => {
        try {
          await client.post('/api/models/bulk', { data: results.data }, {
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-2xl font-semibold">Import Models (CSV)</h2>
      <Card className="p-4">
        <div className="border-dashed border-2 rounded p-6 text-center" onDrop={onDrop} onDragOver={(e)=>e.preventDefault()}>
          ...
          <div className="text-sm text-muted-foreground mt-1">columns: id,name,brandId,brandName,image,bodyType</div>
        </div>
        {preview?.length > 0 && (
          <div className="mt-3 overflow-auto max-h-40 border rounded p-2">
            <table className="w-full text-sm">
              <thead className="text-left text-xs text-muted-foreground">
                <tr>
                  {Object.keys(preview[0]).slice(0, 6).map((h) => <th key={h} className="pr-4">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {preview.map((r, i) => (
                  <tr key={i}>
                    {Object.values(r).slice(0, 6).map((v, j) => <td key={j} className="pr-4">{String(v)}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
