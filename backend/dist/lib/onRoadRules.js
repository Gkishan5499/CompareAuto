...
exports.calcStateCharges = calcStateCharges;
exports.calculateOnRoadPrice = calculateOnRoadPrice;
function pct(amount, rate) {
...
function calcInsurance(input) {
    ...
    const upperState = ((_a = stateCode === null || stateCode === void 0 ? void 0 : stateCode.toUpperCase) === null || _a === void 0 ? void 0 : _a.call(stateCode)) || "";
    if (upperState === "KA") {
        basePercent += 0.003;
...
function calcStateCharges(input) {
...
function calcTcs(exShowroom) {
    if (exShowroom >= 1000000) {
        return Math.round(exShowroom * 0.01);
    }
    return 0;
}
function calcFastag(stateCode, city) {
    const code = stateCode.toUpperCase();
    if (code === "KA")
        return 600;
    if (code === "MH") {
        if (city && city.toLowerCase().includes("pune"))
            return 750;
        return 500;
    }
    return 500;
...
function calculateOnRoadPrice(input) {
    var _a, _b, _c;
    ...
    const mcd = (_a = stateCharges.mcd) !== null && _a !== void 0 ? _a : 0;
    const roadSafetyCess = (_b = stateCharges.roadSafetyCess) !== null && _b !== void 0 ? _b : 0;
    const registrationFee = (_c = stateCharges.registrationFee) !== null && _c !== void 0 ? _c : 0;
    const otherCharges = tcs + fastag;
    const rtoTotal = stateCharges.rto + mcd + roadSafetyCess + registrationFee;
    const onRoad = input.exShowroom +
        rtoTotal +
        insurance.premium +
        tcs +
        fastag;
