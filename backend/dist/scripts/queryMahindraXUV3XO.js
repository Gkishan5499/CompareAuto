...
const queryVariant = async () => {
    try {
        ...
        const carModel = await CarModel_model_1.default.findOne({
            brandName: "Mahindra",
            name: "XUV 3XO"
        });
        if (!carModel) {
            ...
            // Try other patterns
            const allModels = await CarModel_model_1.default.find({ brandName: "Mahindra" }).limit(10);
            console.log("Available Mahindra models:");
            allModels.forEach(m => console.log(`- ${m.name} (id: ${m.id})`));
            const xuv3xoModels = await CarModel_model_1.default.find({
                $or: [
                    { name: /XUV.*3XO/i },
                    { name: /3XO/i }
                ]
        ...
        const variants = await Variant_model_1.default.find({ modelId: carModel.id });
